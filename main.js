...
ipcMain.handle('download-setup', async () => {
  const preferredDistFile = path.join(process.cwd(), 'dist', 'setup.exe');
  const legacyDistFile = path.join(process.cwd(), 'dist', `AutoType-Setup-${app.getVersion()}.exe`);
  const possibleDistFile = fs.existsSync(preferredDistFile) ? preferredDistFile : legacyDistFile;
